alias aws-get-p2='export instanceId=`aws ec2 describe-instances --filters "Name=instance-state-name,Values=stopped,Name=instance-type,Values=p2.xlarge" --query "Reservations[1].Instances[0].InstanceId"` && echo $instanceId'
...
alias aws-ssh='ssh -i aws-key.pem ubuntu@$instanceIp'
